...
| `EMERGE_SYNTHESIS_COMMAND`  | Optional reverse-flywheel provider command. Receives `SynthesisJob` JSON on stdin and returns synthesized pipeline JSON on stdout. Unset records `synthesis_unconfigured`. | —                           |
| `EMERGE_SYNTHESIS_TIMEOUT_S` | Timeout for the provider command, separate from `EMERGE_EXEC_TIMEOUT_S` and runner HTTP timeout | `120`                       |
...
| Distiller                | `scripts/distiller.py` — canonicalizes detected pattern names; runtime synthesis uses this normalization before building `SynthesisJob` records.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Synthesis agent          | `scripts/synthesis_agent.py` — reverse flywheel queue/provider boundary. Owns `pattern_pending_synthesis` / `synthesis_job_ready`, calls a configured provider (`EMERGE_SYNTHESIS_COMMAND`) or records `synthesis_unconfigured`, then routes successful code through `icc_exec` WAL metadata (`source=reverse_flywheel_synthesis`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
