...
**Emerge** solves a core problem: AI operators repeat the same work but do not learn from it, so every session re-reasons from scratch. It uses a **dual flywheel** to crystallize repeated work into deterministic pipelines: a **forward flywheel** (`icc_exec`/`icc_span_open` tracking → policy promotion explore→canary→stable → Claude Code lead-agent synthesis → `.py+.yaml` pipelines → zero-LLM execution), and a **reverse flywheel** (`OperatorMonitor` observes human operators → `PatternDetector` emits repetition facts → Claude Code lead agents use Emerge synthesis skills to inspect facts and write verified pipeline artifacts).
...
| Component            | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **EmergeDaemon**     | MCP JSON-RPC control plane: routes tool calls, orchestrates exec, pipelines, policy updates, fact emission, and crystallization fallback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
flowchart TD
  ...
  K[Claude writes pending pipeline artifact]
  H[exec bridge in icc_exec short-circuits to stable pipeline]
  I[span bridge in icc_span_open executes stable pipeline]

  A --> B --> C --> D --> E1
  E1 -->|"attempts >= 5, success >= 90, verify >= 98"| E2
  E2 -->|"attempts >= 15, success >= 95, verify >= 99"| E3
...
| `icc_crystallize`  | Legacy manual WAL crystallization fallback. Auto-crystallize no longer uses the verbatim path; new artifacts come through Claude Code skills that inspect synthesis events and write pending artifacts.                                                                                                                                                |
...
| Synthesis facts          | `scripts/synthesis_events.py` — normalize observed intent names, package reverse/forward synthesis evidence as `pattern_pending_synthesis`, `synthesis_job_ready`, and `forward_synthesis_pending` facts for Claude Code skills. Python does not call an LLM provider, smoke-test synthesized code, or materialize pipelines.                                                                                                                                                                                                                                                                                                                                 |
| Operator monitor         | `scripts/operator_monitor.py` — daemon writes `pattern_observed` to `events/events-{profile}.jsonl`; local observer path writes `local_pattern_observed` to `events/events-local.jsonl`; suggestion aggregation writes `pattern_aggregated` facts; synthesis fact events are appended to the same streams when Claude skills decide to distill them; `watch_emerge.py` streams these files to watcher agents (old `pattern-alerts-{profile}.json` removed).                                                                                                                                                                                                                                                               |
...
| 🟢 shipped  | **Operator Intelligence Loop** A reverse flywheel that observes the *human*, not just the AI. Runner/local events feed `PatternDetector`; repeated windows emit `pattern_observed` / `local_pattern_observed` facts, and Claude Code skills can package selected windows as `pattern_pending_synthesis` / `synthesis_job_ready` facts. Skills perform distillation and use the normal mechanism tools to verify and write pending artifacts. Operator as author, not executor. |
...
| **Synthesis facts**   | Auditable event payloads built from pattern windows or WAL samples. They load connector `NOTES.md` and optional `synthesis_hints.yaml` and tell Claude Code skills what evidence to inspect; Python does not perform the intelligent synthesis step.                                                                                                                                                                                                  |
