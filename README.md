...
**Emerge** solves a core problem: AI operators repeat the same work but do not learn from it, so every session re-reasons from scratch. It uses a **dual flywheel** to crystallize repeated work into deterministic pipelines: a **forward flywheel** (`icc_exec`/`icc_span_open` tracking → policy promotion explore→canary→stable → Claude Code lead-agent synthesis → smoke-tested `.py+.yaml` pipelines → zero-LLM execution), and a **reverse flywheel** (`OperatorMonitor` observes human operators → `PatternDetector` detects repetition → Claude Code lead agent uses Emerge synthesis skills → coordinator smoke-tests and materializes the pipeline).

**Emerge** is a Claude Code plugin that implements a **dual flywheel**: repeated AI work is tracked via `icc_exec` (ad-hoc code) and `icc_span_open/close` (intent spans), promoted through a **policy registry** (explore → canary → stable), and distilled into connector pipelines that execute with zero LLM inference once stable. Synthesis happens at compile time through Claude Code lead-agent workflows and project skills; runtime bridge execution never calls an LLM.
...
| Component            | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **EmergeDaemon**     | MCP JSON-RPC control plane: routes tool calls, orchestrates exec, pipelines, policy updates, synthesis job submission, and crystallization.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
flowchart TD
  ...
  G([forward_synthesis_pending job])
  J[Claude Code lead agent uses emerge synthesis skill]
  K[icc_synthesis_submit smoke test and materialize pipeline]
...
| `EMERGE_EXEC_TIMEOUT_S`     | Wall-clock cap (seconds) for each `icc_exec` call                                             | `600`                       |
...
| `EMERGE_SYNTHESIS_COMMAND`  | Legacy compatibility provider command for explicit `provider_exec` mode. The default path uses Claude Code lead-agent skills and `icc_synthesis_submit`. | —                           |
| `EMERGE_SYNTHESIS_TIMEOUT_S` | Timeout for the legacy provider command, separate from `EMERGE_EXEC_TIMEOUT_S` and runner HTTP timeout | `120`                       |
| `EMERGE_FORWARD_SYNTHESIS_AUTO_APPROVE` | `1` writes accepted forward synthesis directly to the active pipeline directory; unset keeps artifacts under `_pending/` for review | off                         |
...
| `EMERGE_MAX_REQUEST_BYTES`  | Max HTTP request body size before route parsing                                               | `16777216` (16 MiB)         |
| `EMERGE_HTTP_MAX_CONNECTIONS` | Max concurrent HTTP request handler threads per daemon/runner server                         | `64`                        |
| `EMERGE_RUNNER_EVENT_BUFFER_MAX` | Max per-runner in-memory event window retained for pattern detection                     | `200`                       |
...
| `icc_span_close`   | Close the current span and commit to the span WAL. At stable, auto-activates the generated pipeline by default (writes `.py` + `.yaml` into the real connector directory); set `EMERGE_REQUIRE_APPROVE=1` to keep the manual `_pending/` approval path.                                                                            |
| `icc_span_approve` | Manual approval path for `EMERGE_REQUIRE_APPROVE=1`: move `_pending/<name>.py` to the real pipeline directory and generate YAML metadata, then activate the span bridge.                                                                                                                                                           |
| `icc_exec`         | Execute Python in a persistent local session. Tracks `intent_signature` for flywheel policy. Optionally routes to a remote runner when `target_profile` is mapped — local is the default. Supports `result_var` to return structured JSON from a named global variable. At `synthesis_ready`, emits a forward synthesis job for Claude Code lead-agent distillation. |
| `icc_crystallize`  | Legacy manual WAL crystallization fallback. Auto-crystallize no longer uses the verbatim path; new artifacts should come through lead-agent synthesis and `icc_synthesis_submit`.                                                                                                                                                |
| `icc_synthesis_submit` | Submit a structured result from `emerge-forward-synthesis` or `emerge-reverse-synthesis`; daemon validates it, runs smoke via `icc_exec`, and writes `_pending/` pipeline artifacts unless `EMERGE_FORWARD_SYNTHESIS_AUTO_APPROVE=1`.                                                                                         |
...
| `runner_notify`    | Show a popup on a connected runner machine and wait for operator response (`choice`/`input`/`confirm`/`info`). `toast` is fire-and-forget and returns immediately.                                                                                                                                                                 |
...
| Synthesis coordinator    | `scripts/synthesis_agent.py` + `scripts/synthesis_coordinator.py` — normalize observed intent names, package reverse/forward synthesis jobs for Claude Code lead-agent skills, accept `icc_synthesis_submit` results, validate shape, run smoke checks, and materialize `_pending/` pipeline artifacts. `EMERGE_SYNTHESIS_COMMAND` remains only for explicit legacy `provider_exec` compatibility.                                                                                                                                                                                                                                                                                                                                 |
| Operator monitor         | `scripts/operator_monitor.py` — daemon writes `pattern_alert` to `events/events-{profile}.jsonl`; local observer path writes `local_pattern_alert` to `events/events-local.jsonl`; `SynthesisAgent` writes the pending/ready synthesis events; `watch_emerge.py` streams these files to watcher agents (old `pattern-alerts-{profile}.json` removed).                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Slash commands           | `commands/` (`init`, `cockpit`, `monitor`, `runner-status`, `import`, `export`, `hub`, plus admin flows such as `admin-batch-update-runners` and `diagnose-stuck-flywheel`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Skills                   | Product skills live in `skills/`: initialization, operator-flow distillation, remote runner development, monitor/debug/policy/reflection guidance, lead-agent synthesis (`emerge-forward-synthesis`, `emerge-reverse-synthesis`), cockpit rendering, runner elicitation policy, and admin runner operations. `.cursor/skills/` is not a product delivery location.                                                                                                                                                                                                                                                                                                                                                             |
...
- **Bridge outcome writes are non-stage**: `_try_flywheel_bridge` hands every outcome (exception raised, `verification_state=="degraded"`, `action_result.ok is False`, or `read` empty after a non-empty baseline) to `PolicyEngine.record_bridge_outcome`, which writes `bridge_failure_streak`, `has_ever_returned_non_empty`, and the `last_demotion` / `last_transition_reason` snapshot — but never touches `attempts` / `successes`. After `BRIDGE_BROKEN_THRESHOLD` (default 2) consecutive failures on a `stable` intent, the threshold block forces `stable → canary` with reason `bridge_broken` (exception / verify-degraded / action-not-ok) or `bridge_silent_empty` (empty-regression). `apply_evidence` remains the unique writer for attempt counters — bridge failures are observed separately, not double-counted.
...
**Overview** · policy posture & rollout   |   **Monitors** · connected runner health & last alert   |   **State** · L3 diagnostics and intent-linked timeline   |   **Session** · WAL/checkpoint/recovery controls

Tip: click any image to open full size.
...
| 🟢 shipped  | **Operator Intelligence Loop** A reverse flywheel that observes the *human*, not just the AI. Runner/local events feed `PatternDetector`; repeated windows emit `pattern_alert` plus `pattern_pending_synthesis`; `SynthesisAgent` uses a configured provider to generate replayable Python once, then existing WAL/policy/crystallization turns it into a gated pipeline. Missing providers fail durably as `synthesis_unconfigured` rather than pretending automation happened. Operator as author, not executor. |
...
| **SynthesisAgent**   | Reverse flywheel synthesis coordinator. Builds auditable `SynthesisJob` payloads from pattern windows, loads connector `NOTES.md` and optional `synthesis_hints.yaml`, calls the configured provider, and feeds successful code through `icc_exec` so WAL, PolicyEngine, and PipelineCrystallizer remain the only promotion path.                                                                                                                                                                                                  |
| **Synthesis provider** | External command configured by `EMERGE_SYNTHESIS_COMMAND`. It receives job JSON on stdin and returns `{connector, mode, pipeline_name, code, confidence}` JSON on stdout. Missing or failing providers write durable `synthesis_unconfigured` / `synthesis_failed` events.                                                                                                                                                                                                 |
