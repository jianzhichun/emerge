...
![Version](https://img.shields.io/badge/version-v0.3.130-blue)
Python
License
Tests
...
| Synthesis agent          | `scripts/synthesis_agent.py` — reverse flywheel queue/provider boundary. Emits `pattern_pending_synthesis`, calls a configured provider (`EMERGE_SYNTHESIS_COMMAND`) or records `synthesis_unconfigured`, then routes successful code through `icc_exec` WAL metadata (`source=reverse_flywheel_synthesis`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Operator monitor         | `scripts/operator_monitor.py` — daemon writes `pattern_alert` + `pattern_pending_synthesis` to `events/events-{profile}.jsonl`; local observer path writes `local_pattern_alert` + pending synthesis to `events/events-local.jsonl`; `watch_emerge.py` streams these files to watcher agents (old `pattern-alerts-{profile}.json` removed).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
