...
\title{Emerge: A Muscle-Memory Flywheel for\\Crystallizing AI Agent Execution Patterns}
...
\begin{abstract}
We present \textbf{Emerge}, a plugin for Claude Code that implements a \emph{muscle-memory flywheel} for AI agents.
Repeated tool invocations are tracked by semantic intent (\texttt{intent\_signature}), promoted through a
three-stage policy lifecycle (\textsc{explore} $\to$ \textsc{canary} $\to$ \textsc{stable}),
and eventually \emph{crystallized} into deterministic connector pipelines.
Once a pattern reaches the \textsc{stable} stage, a \emph{flywheel bridge} short-circuits future calls
at the tool layer with zero LLM inference overhead.
We further introduce the \textbf{Operator Intelligence Loop}---a reverse flywheel that observes human operator
behavior, identifies repetitive patterns, and progressively offloads mechanical work to the AI layer.
Together, these mechanisms form a self-improving execution substrate: the agent grows faster and more autonomous
over time, while human operators are progressively freed from work that code can replace.
Emerge is implemented as an open-source Claude Code plugin with 134 passing tests.
\end{abstract}
...
Progressive delivery systems~\cite{limoncelli2018progdelivery} gradually shift traffic to new software versions
while monitoring for regressions.
We adapt this to AI-generated patterns: a candidate pattern enters canary rollout at 20\% before earning full
trust at stable (100\%).
This limits the blast radius of a pattern that appeared reliable in exploration but degrades under production
distribution.
...
\paragraph{Canary and progressive delivery.}
Limoncelli~\cite{limoncelli2018progdelivery} surveys progressive delivery in software systems.
We apply canary rollout semantics to AI-generated code patterns---to our knowledge the first such application.
...
Y. Bang, S. Cahyawijaya, N. Lee, W. Dai, D. Su, B. Wilie, H. Lovenia, Z. Ji, T. Yu, W. Chung, Q. Do,
Y. Xu, and P. Fung.
\newblock {GPTCache}: A Data or Model Infrastructure for Large Language Models.
\newblock \emph{arXiv:2306.05212}, 2023.
...
\bibitem{limoncelli2018progdelivery}
T. Limoncelli.
\newblock Progressive Delivery: How to Ship Software.
\newblock \emph{Queue}, 16(4), 2018.
...
\newblock \emph{arXiv:1709.01885}, 2017.
