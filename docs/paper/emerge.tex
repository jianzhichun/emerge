...
\begin{tikzpicture}[
  ...
  \node[box] (pr) at (-2.6,-0.4) {Intent Registry};
...
artifact: entries in the intent registry.
...
\begin{tikzpicture}[
  ...
  \draw[arr] (cr) -- (reg);
\end{tikzpicture}
\caption{Both AI and operator patterns converge to the same intent registry.}
...
Two files: \texttt{state/sessions/<session\_id>/candidates.json} (per-session,
per-\texttt{intent\_signature} counters) and
\texttt{state/registry/intents.json} (global, per-intent lifecycle state).
